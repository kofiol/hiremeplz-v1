...
export function SettingsDialog({
  ...
  const loadSettings = React.useCallback(async () => {
    ...
    try {
      ...
      setPlatforms(
        new Set((body.preferences.platforms ?? ["upwork", "linkedin"]).filter(
  ...
  const payload = React.useMemo(() => {
    const hourlyMinNumber =
      hourlyMin.trim().length > 0 ? Number(hourlyMin) : null
    const hourlyMaxNumber =
      hourlyMax.trim().length > 0 ? Number(hourlyMax) : null
    ...
    return {
      ...
      preferences: {
        ...
        fixedBudgetMin: Number.isFinite(fixedBudgetMinNumber)
          ? fixedBudgetMinNumber
          : null,
      },
      agent: {
        timeZones,
  ...
  return (
    <Dialog open={open} onOpenChange={setOpen}>
      ...
      <DialogContent className="max-w-[calc(100%-2rem)] overflow-hidden p-0 sm:max-w-[780px]">
        <div className="grid max-h-[80vh] grid-cols-1 sm:grid-cols-[220px_1fr]">
          <aside className="bg-muted/30 border-b p-3 sm:border-b-0 sm:border-r sm:p-4">
